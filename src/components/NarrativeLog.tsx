  ...
  return (
    <div className="space-y-2">
      <div className="flex items-center justify-between">
        <h3 className="text-xs font-medium text-white/50 uppercase tracking-wider">事件日志</h3>
        <span className="text-[10px] text-white/30 font-mono">{narrativeLog.length} 条</span>
      </div>
      <div className="max-h-[320px] min-h-[120px] overflow-y-auto space-y-1.5 p-4 
                      ws-surface rounded-xl text-sm">
        {narrativeLog.length === 0 && (
          <div className="text-white/30 text-xs space-y-1.5 py-4 text-center">
            <p className="italic">世界尚未开始演化...</p>
            <p className="text-[10px] text-white/20">提示：按 1-5 快速选择行动，按 / 输入自定义行动</p>
          </div>
        )}
        {narrativeLog.map((entry, i) => {
          ...
          if (entry.type === 'system') {
            return (
              <div key={i} className="text-white/30 text-xs font-mono leading-5 pl-0">
                {entry.text}
              </div>
            )
          }
          ...
          return (
            <div key={i} className="text-white/80 leading-6 flex items-start gap-1.5">
