...
function getDefaultFallbackChoices(mode: ScenarioMode): string[] {
  switch (mode) {
    case 'training':
      return [
        '[正面突破] 直接向核心决策者提出你的具体方案（含数字和时间线），要求其当场表态同意或提出具体反对理由',
        '[迂回包抄] 绕过当前僵持的议题，先在次要议题上与某一方达成共识建立信任，再用这个小胜利作为筹码回到主战场',
        '[制造紧迫] 引入一个外部时间约束或竞争威胁（如"竞品下周发布""监管下月检查"），迫使所有人加速决策而非继续拖延',
      ]
    case 'simulation':
      return [] // 仿真模式不需要人工选项
    case 'game':
