...
3. choices 设计原则（核心！每个选项必须让局面产生不可逆的实质推进）：
   - 每个选项是一个完整的策略方案，必须包含三要素：【对谁做】+【具体怎么做】+【预期产生什么后果】
   - 格式："[策略名称] 对[具体对象]采取[具体手段]，预期[具体后果]"
   - 正确示例："[以退为进] 向CFO主动提出缩减10%预算换取Q2项目自主审批权，同时要求对方BU按完成率比例承担削减，迫使对方暴露真实底线"
   - 正确示例："[数据施压] 当场展示你准备的ROI对比表（你的BU 115% vs 对方85%），直接质问CFO'高绩效部门承担更大削减是否符合公司价值观'，制造公开压力"
   - 正确示例："[联盟策略] 会后30分钟内单独约CEO茶歇，带着两个替代方案（A保项目砍人力/B保人力延期项目）让CEO做选择题，绕过CFO的一刀切逻辑"
   - 每个选项至少30字，必须包含具体的人名/角色名、具体的行动方法、可预见的后果
   - 三个选项之间必须代表截然不同的策略路径，选择任何一个都会导致局面向不同方向发展
   - 绝对禁止的空洞选项（出现即为失败输出）：
     * "观察局势""继续等待""了解情况""看看反应""进一步沟通""保持关注"
     * "收集更多信息""评估风险""权衡利弊""寻求共识"
     * 任何不包含具体对象和具体手段的抽象描述
   - 每轮的选项必须基于上一轮的结果推进，不允许重复之前出现过的策略方向
...
8. 情景推进强制规则（防止原地踏步）：
   - 每一轮的 narrative 必须引入至少一个新信息（新事实/新角色反应/新外部事件/新约束条件）
   - 禁止连续两轮描述相同的博弈态势而无实质变化
   - 每2-3轮必须出现一次"局面升级"：新的利益相关方介入、截止时间逼近、外部事件冲击、某方态度突变
   - 角色的态度变化必须有因果链：你做了X → 角色观察到Y → 角色态度从A变为B
   - 如果用户连续选择保守策略，系统必须通过外部事件制造压力迫使决策（如"媒体已经发布报道""竞争对手出价了""老板要求今天给答复"）`,
    agentTickEnabled: true,
    showMap: false,
    showScore: true,
    autoRun: false,
...
8. 推演推进强制规则（防止仿真停滞）：
   - 每轮必须有至少一个Agent做出与上一轮不同的决策行为
   - 禁止连续两轮出现相同的系统状态描述
   - Agent之间的交互必须产生可观测的状态变化（态度/策略/资源/信息）
   - 每2-3轮必须出现一次系统层面的涌现现象（联盟形成/冲突爆发/策略转向/外部冲击响应）
   - 关键变量（如冲突指数、协作指数）每轮必须有变化，不允许连续3轮保持不变`,
    agentTickEnabled: true,
    showMap: false,
    showScore: false,
    autoRun: true,
